...
impl InputSystem {
    ...
    pub fn process_events(&mut self, window: &glutin::Window, events_loop: &glutin::EventsLoop) -> bool {
        //Processes Input and stores it in the keymap struct
        //returns true if should close
        //Usin the new glutin 0.8 methode
        ...
        events_loop.poll_events(|event| {
            match event {
                glutin::Event::WindowEvent{window_id, event} => match event {
                    glutin::WindowEvent::KeyboardInput(State, _, Key,_) => {
                        match State {
                            glutin::ElementState::Pressed => {
                                match Key {
                                    Some(glutin::VirtualKeyCode::Escape) => return_value = true,
